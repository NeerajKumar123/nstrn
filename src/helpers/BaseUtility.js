...
import {SALT_KEY} from '../../src/constants/StaticValues';
...
const updateSelectionData = res => {
  ...
  const allFiledYearsData = allYearsData?.filter(
    element => element?.tax_file_status_id == 16,
  );
...
export const loadIntialData = callback => {
  ...
  SKTStorage.loadLastSavedData(res => {
    ...
    global[keyProvince] = valueProvince
      ? JSON.parse(valueProvince)
      : {state_id: 51, state_name: 'Alberta', short_name: 'AB'};
      ...
      if (fileStatusRes?.status == 1) {
        ...
        incorpGetIncorpStatus(params, incStatusRes => {
          if (incStatusRes?.status == 1) {
            const incStatusData =
              incStatusRes?.data && incStatusRes?.data.length > 0
                ? incStatusRes?.data[0]
                : {};
            global.incStatusData = incStatusData
              ? {...incStatusData, ...global.userInfo}
              : {...global.onlineStatusData, ...global.userInfos};
            taxDocsGetTaxDocsStatus(params, taxDocsRes => {
              if (taxDocsRes?.status) {
                const taxDocsResData =
                  taxDocsRes?.data && taxDocsRes?.data.length > 0
                    ? taxDocsRes?.data[0]
                    : {};
                global.taxDocsStatusData = taxDocsResData
                  ? {...taxDocsResData, ...global.userInfo}
                  : {...global.taxDocsResData, ...[]};
                const {user_id} = global.userInfo;
                const params = {User_Id: user_id};
                craLattersGetStatus(params, craRes => {
                  if (craRes?.status == 1) {
                    const craLattersResData =
                      craRes?.data && craRes?.data.length > 0
                        ? craRes?.data[0]
                        : {};
            ...
            const msg =
              incStatusRes?.message ?? 'Something went wrong,Please try again.';
            Alert.alert('SukhTax', msg);
            callback({status: -1, msg: msg});
          }
        });
        ...
        const msg =
          fileStatusRes?.message ?? 'Something went wrong,Please try again.';
        Alert.alert('SukhTax', msg);
        callback({status: -1, msg: msg});
      }
    });
...
export const encrypt = originalText => {
  let ciphertext = originalText;
  if (originalText?.length > 0) {
    // Encrypt
    ciphertext = CryptoJS.AES.encrypt(originalText, SALT_KEY).toString();
  }
  console.log(`Encrypt:${originalText} =====> ${ciphertext}`);
  return ciphertext;
};

export const decrypt = ciphertext => {
  let originalText = ciphertext;
  if (ciphertext?.length > 0) {
    // Decrypt
    let bytes = CryptoJS.AES.decrypt(ciphertext, SALT_KEY);
    originalText = bytes.toString(CryptoJS.enc.Utf8);
  }
  console.log(`Decrypt:${ciphertext} =====> ${originalText}`);
  return originalText;
};
